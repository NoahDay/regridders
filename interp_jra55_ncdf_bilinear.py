...
if __name__ == "__main__":
    ...
    time_index     = grid1_ds['time'][:]
    print(grid1_ds)
#    ds = xr.Dataset()
#    coords = ('time', 'lat', 'lon')
#    ds['prsn'] = grid1_ds['prsn']
#    ds.coords['lon'] = lon1
#    ds.coords['lat'] = lat1
...
#                }
    ...
    #grid2_ds        = xr.open_dataset(self.dstgrid)
    #    grid2_ds['lat'] = (['ny','nx'],grid2_ds[lat].values*(180/np.pi),{'units':'degrees_north','_FillValue':-2e8})
    #    grid2_ds['lon'] = (['ny','nx'],grid2_ds[lon].values*(180/np.pi),{'units':'degrees_east','_FillValue':-2e8})
    #    grid2_ds        = grid2_ds.drop(('ulat','ulon','tlat','tlon','clon_t','clat_t','clat_u','clon_u','angle','uarea'))
    #    grid2_ds        = grid2_ds.assign_coords(xt_ocean=grid2_ds['lon'][0,:],yt_ocean=grid2_ds['lat'][:,0])
    #    grid2_ds        = grid2_ds.set_index(nx='xt_ocean',ny='yt_ocean')
    #tlon2    = grid2_ds["lon"][:,:] # 2D. Assumed ULON in degrees
    #tlat2    = grid2_ds["lat"][:,:] # 2D. Assumed ULAT in degrees
    ...
    method = 'bilinear'
    periodic = True
    blin_grid_name = 'bilinear_jra55_om2_025deg_py.nc'
